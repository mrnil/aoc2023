puzzleinput ="""Game 1: 1 blue, 8 green; 14 green, 15 blue; 3 green, 9 blue; 8 green, 8 blue, 1 red; 1 red, 9 green, 10 blue
...
puzzleinput = puzzleinput.splitlines()
gamenumbertotal = 0
totalpower = 0
for gameline in puzzleinput:
    print(gameline)
    gamenumber = int(gameline.split(":")[0].split()[1])
    gamedata = gameline.split(":")[1].split(";")
    cubes = {"red":0,"green":0,"blue":0}
    for thisgame in gamedata:
        data = thisgame.split(",")
        for picked in data:
            pickeddata = picked.split()
            colourname = pickeddata[1]
            if int(pickeddata[0]) > int(cubes[colourname]):
                cubes[colourname] =int(pickeddata[0])
    gamepower = cubes["red"]*cubes["green"]*cubes["blue"]
...
print(gamenumbertotal, totalpower)
