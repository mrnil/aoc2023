...
total = 0
for thisLine in inputtext:
    thisNumberString = ''
    end = 0
    index = inputtext.index(thisLine)
    for i in range(len(thisLine)):
        if thisLine[i].isnumeric():
            thisNumberString = thisNumberString+thisLine[i]
        else:
            for thisword in wordnumbers:
                end = i+len(thisword)
                if end > len(thisLine):
                    end = len(thisLine)-1
                if (thisword == thisLine[i:end]):
                    thisNumberString = thisNumberString+numbernumbers[wordnumbers.index(thisword)]

    first = thisNumberString[0]
    last = thisNumberString[len(thisNumberString)-1]
    thisNumb = "".join([first,last])
    total=total+int(thisNumb)    
    print (thisLine,thisNumberString,thisNumb)
